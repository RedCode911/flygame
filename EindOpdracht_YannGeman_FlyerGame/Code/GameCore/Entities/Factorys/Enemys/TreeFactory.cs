...
namespace Code.GameCore.Entities.Factorys.Enemys
{
    ...
    {
        public static TreeSprite CreateBig(Texture2D texture, float x, float y, float speed, float baseScale)
        {
            return Create(texture, new Vector2(x, y), speed, baseScale * 1.5F);
        }

        public static TreeSprite CreateSmall(Texture2D texture, float x, float y, float speed, float baseScale)
        {
            return Create(texture, new Vector2(x, y), speed, baseScale * 0.75F);
        }

        public static TreeSprite Create(Texture2D texture, float x, float y, float speed, float scale)
        {
            return Create(texture, new Vector2(x, y), speed, scale);
        }

        public static TreeSprite Create(Texture2D texture, Vector2 position, float speed, float scale)
        {
